...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    // First, delete all non-owner users and the old admin account
    await this.pool.query(
      "DELETE FROM users WHERE role != 'owner'"
    );
    await this.pool.query(
      "DELETE FROM users WHERE email = 'admin@rupp.edu.kh'"
    );
    ...
    this.logger.log(`Ensured schema "${this.schema}" and tables exist.`);
  }
